...
{
    public bool bossDeathWinsGame = false;
    public GameObject boss;


    // Whether the game is winnable by surviving (for level 2)
    [Tooltip("Whether the game is winnable by surviving or not\nDefault: true for level 2")]
    public bool gameIsWinnableBySurvival = true;

    // Duration the player should survive in seconds
    public float winningDuration = 60f;
    ...
    private void Start()
    {
        ...
        if (gameIsWinnableBySurvival)
        {
            InvokeRepeating("SpawnEnemies",1.2f,1.2f);
        }
    ...
    private void Update()
    {
        //Check if the game is winnable and the player has won
        if(!gameWon && gameIsWinnableBySurvival && Time.timeSinceLevelLoad >= winningDuration)
        {
            WinGameBySurvival();
            LevelCleared();
        }
        ...
        //if(boss == null)
        //{
        //    Debug.Log("Boss reference is null. Checking if boss GameObject is destroyed.");
        ...
        //if (!boss.activeSelf)
        //{
        //    Debug.Log("Boss is inactive. Game over condition met.");
        //    // Trigger game over condition if the boss is inactive
        //    GameOver();
        //}
        if (bossDeathWinsGame && boss != null && !boss.activeSelf)
        {
            Debug.Log("Boss is inactive. Game win condition met.");
            GameOver();
        }
    ...
    void SpawnEnemies()
    {
        if (!gameWon)
        {
            ...
            GameObject randomEnemyPrefab = enemyPrefabs[Random.Range(0,8)];
            // Spawn the chosen enemy
            Instantiate(randomEnemyPrefab, GetRandomSpawnPosition(), Quaternion.identity);
        }            
    }
    ...
    Vector3 GetRandomSpawnPosition()
    {
        ...
        float randomX = Random.Range(11f, -32f); // Adjust according to your screen size
        float randomY = Random.Range(25f, -5f); // Adjust according to your screen size
        return new Vector3(randomX, randomY, 0);
    }
    public void WinGameBySurvival()
    {
        gameWon = true;
        ...
        Debug.Log("Game Over! You survived for " + winningDuration + " seconds.");    
    }
