...
public class Health : MonoBehaviour
{
    ...
    public virtual void TakeDamage(int damageAmount)
    {
        ...
        else
        {
            ...
            if (currentHealth > 0 && currentHPIndex < hpIndicators.Length)
            {
                hpIndicators[currentHPIndex].SetActive(false);
                currentHPIndex++;
            }

            if (boss != null && boss == gameObject && currentHealth <= 0)
            {
                // Invoke the bossDestroyedEvent if the boss is destroyed
                bossDestroyedEvent.Invoke();
            }
        }
        if(bossHealthBar != null)
        {
            bossHealthBar.UpdateHealthUI();
        }
    }
